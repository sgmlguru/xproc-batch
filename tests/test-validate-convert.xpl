...
<p:declare-step
    ...
    <sgproc:validate-convert>
        <p:with-input port="manifest" href="04/test-xproc-manifest.xml"/>
        <p:with-input port="sch" href="sch/test-para.sch"/>
        <p:with-option name="input-base-uri" select="resolve-uri('./04/sources')"/>
        <!--<p:with-option name="include-filter" select="'\.xml'"/>-->
        <p:with-option name="output-base-uri" select="resolve-uri('./04/tmp/out')"/>
        <p:with-option name="reports-dir" select="resolve-uri('./04/tmp/reports')"/>
        <p:with-option name="tmp-dir" select="resolve-uri('./04/tmp')"/>
