...
if which saxon > /dev/null 2>&1 && saxon --help | grep "EXPath Packaging" > /dev/null 2>&1; then
    ...
    echo
    xslt() {
        java -cp "$CP" net.sf.saxon.Transform ${CATALOG:+"$CATALOG"} -config:${XSPEC_HOME}/../saxon/saxon9/saxon-config.xml "$@"
    }
    xquery() {
        java -cp "$CP" net.sf.saxon.Query ${CATALOG:+"$CATALOG"} -config:${XSPEC_HOME}/../saxon/saxon9/saxon-config.xml "$@"
    }
fi
...
SAXON_CP=${XSPEC_HOME}/../saxon/saxon9/saxon9ee.jar:${XSPEC_HOME}/../saxon/saxon9/xml-resolver-1.2.jar:${XSPEC_HOME}/../saxon/saxon9/kg-xslt-extensions.jar:${XSPEC_HOME}/../nj-id-mapper/nj-id-mapper-1.11.6.jar
